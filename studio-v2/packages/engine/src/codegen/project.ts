...
import type { ProjectDocument, NavigationConfig, DataQuery } from "@flipova/studio-core";
...
function generateAppEntry(
  ...
  let sbStyle = sb.style || 'auto';
  if (sbStyle === 'auto' && sb.backgroundColor && sb.backgroundColor.startsWith('#')) {
    const c = sb.backgroundColor.replace('#', '');
    if (c.length === 3 || c.length === 6) {
      const fullC = c.length === 3 ? c.split('').map((x: string) => x + x).join('') : c;
      const r = parseInt(fullC.substring(0, 2), 16);
      const g = parseInt(fullC.substring(2, 4), 16);
      const b = parseInt(fullC.substring(4, 6), 16);
      const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
      sbStyle = luminance > 0.5 ? 'dark' : 'light';
    }
  }
  const sbTag = `\n        <StatusBar style="${sbStyle}"${sb.hidden ? ' hidden' : ''}${sb.translucent !== undefined ? ` translucent={${sb.translucent}}` : ''}${sb.backgroundColor ? ` backgroundColor="${sb.backgroundColor}"` : ''} />`;
...
${rootStackScreens.join('\n')}
        ...
        </NavigationContainer>${gsClose}${sbTag}
      </FoundationProvider>
    </SafeAreaProvider>
  );
}
...
function generateRootLayout(project: ProjectDocument): string {
  ...
  const hasStatusBar = sb && (sbStyle !== 'auto' || sb.hidden || sb.backgroundColor);
  const sbImport = hasStatusBar ? `\nimport { StatusBar } from "expo-status-bar";` : "";
  const sbTag = hasStatusBar ? `\n        <StatusBar style="${sbStyle}"${sb.hidden ? ' hidden' : ''}${sb.translucent ? ' translucent' : ''}${sb.backgroundColor ? ` backgroundColor="${sb.backgroundColor}"` : ''} />` : "";
