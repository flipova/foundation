...
import { colors as DS, font } from '../../ds';
import {  TreeNode  } from '@flipova/studio-core';
import { useStudio } from '../../useStudio';
import { Feather } from '@expo/vector-icons';
import { deriveSlotConfig } from '@flipova/studio-core';
...
// Studio design system — replaces per-component useTheme calls
...
const MODE_BADGE_CONFIG = {
  static:   { label: 'static',   color: C.textSub, bg: C.surface2 },
  template: { label: 'template', color: C.text, bg: C.surface2 },
  data:     { label: 'data',     color: C.primary, bg: C.surface2 },
} as const;
  ...
  return (
    <>
      ...
      <Pressable style={[s.row, isSel && s.rowSel, isMoving && s.rowMoving, { paddingLeft: 8 + depth * 14 }]} onPress={() => setSel(node.id)}>
        ...
        <Feather name={kindIcon(node.kind) as any} size={11} color={isSel ? C.accent : C.muted} />
        <Text style={{ ...s.name as any, ...(isSel ? { color: C.accent } : {}) }} numberOfLines={1}>{node.registryId}</Text>
        {node.slotName && <Text style={s.slotBadge}>{node.slotName}</Text>}
        {node.conditionalRender && (
          <Tooltip text={LAYERS_TEXTS.tooltipCondition}>
            <Feather name="eye" size={9} color={C.accent} style={s.indicatorIcon} />
          </Tooltip>
        )}
        {!hideSecondaryBadges && node.repeatBinding && (
          <Tooltip text={LAYERS_TEXTS.tooltipRepeat}>
            <Feather name="repeat" size={9} color={C.textSub} style={s.indicatorIcon} />
          </Tooltip>
        )}
        {node.events && Object.values(node.events).some((a: any) => a?.length > 0) && (
          <Tooltip text={LAYERS_TEXTS.tooltipEvent}>
            <Feather name="zap" size={9} color={C.accent} style={s.indicatorIcon} />
          </Tooltip>
        )}
        {!hideSecondaryBadges && node.bindings && Object.keys(node.bindings).length > 0 && (
          <Tooltip text={LAYERS_TEXTS.tooltipBinding}>
            <Feather name="link" size={9} color={C.textSub} style={s.indicatorIcon} />
          </Tooltip>
        )}
        {isSel && !movingId && depth > 0 && (
          <View style={[s.actions, hideSecondaryBadges && s.actionsCompact]}>
            ...
            <Pressable onPress={() => startMove(node.id)} hitSlop={4}><Feather name="move" size={11} color={C.accent} /></Pressable>
            <Pressable onPress={() => setShowSaveInput(true)} hitSlop={4}><Feather name="save" size={10} color={C.muted} /></Pressable>
            <Tooltip text={LAYERS_TEXTS.tooltipDelete}>
              <Pressable onPress={() => setShowDeleteConfirm(true)} hitSlop={4}><Feather name="trash-2" size={10} color={C.muted} /></Pressable>
            </Tooltip>
          </View>
        )}
        {isMoving && <Pressable onPress={cancelMove} hitSlop={6}><Feather name="x-circle" size={12} color={C.accent} /></Pressable>}
      </Pressable>
      ...
      {isDropTarget && (
        <Pressable style={s.dropInto} onPress={() => dropInto(node.id, 0)}>
          <Feather name="corner-down-right" size={10} color={C.accent} />
...
const ScreenRow: React.FC<{ page: any; names: ReturnType<typeof deriveScreenNames>; isActive: boolean; groupId?: string }> = ({ page, names, isActive, groupId }) => {
  ...
  const commitRename = async () => {
    ...
    await renamePage(page.id, trimmed, trimmed.toLowerCase().replace(/\s+/g, '-'));
    setEditing(false);
  };
  ...
  const typeColor: Record<string, string> = {
    tabs: C.muted, drawer: C.muted, stack: C.muted, auth: C.muted, protected: C.muted, custom: C.muted,
  };
            ...
            {allGroups.map((g: any) => {
              ...
              return (
                <Pressable
                  ...
                  disabled={isCurrent}
                >
                  <View style={[s.moveGroupIcon, { backgroundColor: C.surface2 }]}>
...
const GroupRow: React.FC<{ group: any }> = ({ group }) => {
  ...
  const typeColor: Record<string, string> = {
    auth: C.muted, protected: C.muted, tabs: C.muted, drawer: C.muted, stack: C.muted, custom: C.muted,
  };
  ...
  return (
    <View style={s.groupBlock}>
      ...
      <Pressable style={s.groupHeader} onPress={() => setExpanded(e => !e)}>
        ...
        <View style={[s.groupTypeBadge, { backgroundColor: C.surface2 }]}>
...
const LayersPanel: React.FC = () => {
  ...
  return (
    <View style={[s.root, { height: panelHeight, borderTopWidth: 1, borderTopColor: C.border, backgroundColor: C.surface }]}>
        ...
        {(['layers', 'screens'] as const).map(t => (
          <Pressable key={t} style={[s.tab, bottomTab === t && s.tabOn]} onPress={() => setBottomTab(t)}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              ...
              <Text style={[s.tabText, { color: bottomTab === t ? C.primary : C.muted }]}>{t.toUpperCase()}</Text>
            </View>
          </Pressable>
        ))}
      </View>
      {bottomTab === 'layers' ? (
        ...
        <View style={s.scroll}>
          ...
          <View style={{ flexDirection: 'row', padding: 8, gap: 4 }}>
            <Pressable style={[s.createBtn, { flex: 2, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border }]} onPress={() => {
              const newId = 'pg_' + Date.now();
              updateProject({ pages: [...(project as any).pages, { id: newId, name: 'New Screen', root: { id: 'root_'+Date.now(), type: 'View', props: { style: { flex: 1, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' } }, children: [] } }] } as any);
              setPageId(newId);
            }}>
              <Feather name="plus" size={11} color={C.muted} />
              <Text style={[s.createBtnText, { color: C.text }]}>Screen</Text>
            </Pressable>
            <Pressable style={[s.createBtn, { flex: 1, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border }]} onPress={() => {
              window.dispatchEvent(new Event('open-settings'));
            }}>
              <Feather name="settings" size={11} color={C.muted} />
            </Pressable>
            <Pressable style={[s.createBtn, { flex: 1, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border }]} onPress={() => {
              addScreenGroup({ id: 'grp_' + Date.now(), name: 'Group', type: 'stack', screenIds: [] });
            }}>
              <Feather name="folder-plus" size={11} color={C.muted} />
            </Pressable>
            <Pressable style={[s.createBtn, { flex: 1, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border }]} onPress={() => {
            ...
            <Pressable style={[s.createBtn, { flex: 1, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border }]} onPress={() => {
              ...
              <Feather name="shield" size={11} color={C.muted} />
            </Pressable>
          </View>
        </View>
      )}
...
const s = StyleSheet.create({
  ...
  tabOn: { borderBottomWidth: 1 },
  tabText: { fontSize: 9, fontWeight: '500', letterSpacing: 0.5, fontFamily: (font?.family || 'Lexend') },
  tabTextOn: { },
  scroll: { flex: 1 },
  row: { flexDirection: 'row', alignItems: 'center', paddingVertical: 4, paddingRight: 6, gap: 4 },
  rowSel: { backgroundColor: C.surface2, borderLeftWidth: 1, borderLeftColor: C.primary },
  rowMoving: { backgroundColor: C.surface2, borderLeftWidth: 2, borderLeftColor: C.primary },
  name: { flex: 1, fontSize: 11, fontWeight: '400', fontFamily: (font?.family || 'Lexend'), color: C.text },
  slotBadge: { fontSize: 8, backgroundColor: C.surface2, color: C.textSub, paddingHorizontal: 4, paddingVertical: 1, borderRadius: 2, fontFamily: (font?.family || 'Lexend') },
  indicatorIcon: { marginLeft: 1 },
  indicatorBadge: { fontSize: 8, color: C.muted },
  ...
  slotLabel: { flex: 1, color: C.muted, fontSize: 10, fontStyle: 'italic', marginLeft: 4, fontFamily: (font?.family || 'Lexend') },
  slotEmpty: { fontSize: 8, color: C.muted, opacity: 0.5, fontStyle: 'italic', fontFamily: (font?.family || 'Lexend') },
  slotCount: { fontSize: 8, color: C.primary, backgroundColor: C.surface2, paddingHorizontal: 4, paddingVertical: 1, borderRadius: 2, fontWeight: '600', fontFamily: (font?.family || 'Lexend') },
  slotRowTargeted: { backgroundColor: C.surface2 },
  slotDropBtn: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: C.surface2, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 3 },
  slotDropText: { fontSize: 8, color: C.primary, fontWeight: '600', fontFamily: (font?.family || 'Lexend') },
  dropLine: { height: 6, marginHorizontal: 8, justifyContent: 'center' },
  dropLineInner: { height: 1, backgroundColor: C.primary, borderRadius: 1 },
  dropInto: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 12, paddingVertical: 3, marginLeft: 20 },
  dropIntoText: { color: C.primary, fontSize: 9, fontWeight: '600', fontFamily: (font?.family || 'Lexend') },
  cancelBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: C.surface2, padding: 6, borderBottomWidth: 1, borderBottomColor: C.border },
  cancelText: { color: C.primary, fontSize: 10, fontWeight: '500', fontFamily: (font?.family || 'Lexend') },
  screenRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12, paddingVertical: 7 },
  screenRowOn: { backgroundColor: C.surface2 },
  screenName: { color: C.text, fontSize: 11, fontWeight: '400', flex: 1, fontFamily: (font?.family || 'Lexend') },
  screenHint: { color: C.muted, fontSize: 9, marginTop: 1, fontFamily: (font?.family || 'Lexend') },
  rowAction: { width: 20, height: 20, borderRadius: 3, alignItems: 'center', justifyContent: 'center' },
  rowActionDanger: { backgroundColor: C.surface2 },
  moveOverlay: { flex: 1, backgroundColor: C.bg, justifyContent: 'center', alignItems: 'center' },
  moveSheet: {
    width: 280, backgroundColor: C.surface, borderRadius: 8,
    borderWidth: 1, borderColor: C.border, overflow: 'hidden',
    boxShadow: '0px 12px 24px rgba(0,0,0,0.6)' as any,
  },
  moveSheetHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, borderBottomWidth: 1, borderBottomColor: C.border },
  moveSheetTitle: { flex: 1, color: C.text, fontSize: 12, fontWeight: '600', fontFamily: (font?.family || 'Lexend') },
  moveSheetSub: { color: C.muted, fontSize: 10, paddingHorizontal: 12, paddingTop: 8, paddingBottom: 4, fontFamily: (font?.family || 'Lexend') },
  moveGroupItem: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, paddingVertical: 10 },
  moveGroupItemCurrent: { opacity: 0.4 },
  moveGroupIcon: { width: 26, height: 26, borderRadius: 4, alignItems: 'center', justifyContent: 'center' },
  moveGroupName: { color: C.text, fontSize: 11, fontWeight: '500', fontFamily: (font?.family || 'Lexend') },
  moveGroupType: { color: C.muted, fontSize: 9, marginTop: 1, fontFamily: (font?.family || 'Lexend') },
  moveGroupCurrentBadge: { backgroundColor: C.surface2, borderRadius: 3, paddingHorizontal: 5, paddingVertical: 2 },
  moveGroupCurrentText: { color: C.muted, fontSize: 9, fontWeight: '600', fontFamily: (font?.family || 'Lexend') },
  renameInput: { height: 22, backgroundColor: C.surface2, borderRadius: 3, borderWidth: 1, borderColor: C.primary, color: C.text, fontSize: 11, paddingHorizontal: 6, fontFamily: (font?.family || 'Lexend') },
  groupSectionTitle: { color: C.muted, fontSize: 8, fontWeight: '700', letterSpacing: 1, paddingHorizontal: 12, paddingTop: 8, paddingBottom: 4, fontFamily: (font?.family || 'Lexend') },
  groupBlock: { borderBottomWidth: 1, borderBottomColor: C.border },
  groupHeader: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 10, paddingVertical: 6 },
  groupName: { color: C.text, fontSize: 11, fontWeight: '500', flex: 1, fontFamily: (font?.family || 'Lexend') },
  groupTypeBadge: { paddingHorizontal: 5, paddingVertical: 1, borderRadius: 3 },
  groupTypeText: { fontSize: 8, fontWeight: '700', fontFamily: (font?.family || 'Lexend') },
  addPageBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 28, paddingVertical: 5 },
  addPageBtnText: { color: C.muted, fontSize: 9, fontWeight: '500', fontFamily: (font?.family || 'Lexend') },
  addPagePicker: { backgroundColor: C.surface2, marginHorizontal: 8, marginBottom: 4, borderRadius: 4, borderWidth: 1, borderColor: C.border, padding: 8, gap: 4 },
  addPagePickerTitle: { color: C.text, fontSize: 10, fontWeight: '600', marginBottom: 4, fontFamily: (font?.family || 'Lexend') },
  addPagePickerEmpty: { color: C.muted, fontSize: 9, fontStyle: 'italic', fontFamily: (font?.family || 'Lexend') },
  addPagePickerItem: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 5, paddingHorizontal: 4 },
  addPagePickerItemText: { color: C.text, fontSize: 11, fontFamily: (font?.family || 'Lexend') },
  createBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, backgroundColor: C.primary, borderRadius: 4, paddingVertical: 6 },
  createBtnText: { color: '#fff', fontSize: 10, fontWeight: '600', fontFamily: (font?.family || 'Lexend') },
  saveRow: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 3, paddingRight: 8 },
  saveInput: { flex: 1, height: 24, backgroundColor: C.surface2, borderRadius: 3, borderWidth: 1, borderColor: C.border, color: C.text, fontSize: 10, paddingHorizontal: 6, fontFamily: (font?.family || 'Lexend') },
  saveBtn: { width: 22, height: 22, borderRadius: 3, backgroundColor: C.primary, alignItems: 'center', justifyContent: 'center' },
  saveCancelBtn: { width: 22, height: 22, borderRadius: 3, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, alignItems: 'center', justifyContent: 'center' },
  emptyText: { color: C.muted, fontSize: 11, fontStyle: 'italic', textAlign: 'center', padding: 16, lineHeight: 16, fontFamily: (font?.family || 'Lexend') },
});
