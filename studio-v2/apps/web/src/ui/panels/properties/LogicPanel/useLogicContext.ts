...
import type { TreeNode } from '@flipova/studio-core';
...
function getRepeatSource(ancestor: TreeNode, child: TreeNode): string | null {
  ...
  if (ancestor.slotBindings) {
    for (const [, binding] of Object.entries(ancestor.slotBindings)) {
      if (binding && (binding as any).mode !== 'static' && (binding as any).source) {
        return (binding as any).source || null;
      }
    }
  }
...
export function useLogicContext(): LogicContext {
  ...
  const repeatAncestor = React.useMemo(() => {
    ...
    return findAncestorWith(pg.root, sel.id, n =>
      !!n.repeatBinding ||
      Object.values(n.slotBindings ?? {}).some((b: any) => b && b.mode !== 'static' && !!b.source)
    );
  }, [pg, sel]);
  ...
  React.useEffect(() => {
    ...
    if (sel.slotBindings) {
      for (const [, binding] of Object.entries(sel.slotBindings)) {
        if (binding && (binding as any).mode !== 'static' && (binding as any).source) fetchForSource((binding as any).source);
      }
    }
