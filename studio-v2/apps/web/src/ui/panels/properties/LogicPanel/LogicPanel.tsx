...
import type { ActionDef, TreeNode, RegItem } from '@flipova/studio-core';
...
// registry meta — resolved via deriveSlotConfig below
const getLayoutMeta = (_id: string): RegItem | null => null;
const getBlockMeta  = (_id: string): RegItem | null => null;
    ...
    <View style={{ gap: 8 }}>
      {inRepeat && itemFields.length > 0 && (
        <View style={s.contextNote}>
          <Feather name="repeat" size={10} color={C.textSub} />
        ...
        return (
          <View key={p.name} style={singleColumn ? { width: '100%' } : undefined}>
            <View style={s.bindRow}>
              <Text
                style={[s.bindPropName, isBound && { color: isItemField ? C.text : C.textSub }, s.bindPropNameTruncated]}
                numberOfLines={1}
                ellipsizeMode="tail"
              >
                {p.label || p.name}
            ...
            <SmartInput
              label=""
              value={bound}
              onChange={(v: any) => onUpdateBindings(sel.id, { [p.name]: v })}
...
const LogicPanel: React.FC = () => {
  ...
  const arraySlotConfig = React.useMemo(() => {
    ...
    const primarySlot = slotCfg.mode === 'items'
      ? { prop: slotCfg.itemsProp ?? 'items', label: ((layoutMeta.slots || []).find((s: any) => s.kind === 'items') as any)?.label ?? 'Items', kind: 'items' as const }
      : null;
      ...
      >
        ...
        {showEventPicker ? (
          ...
          <Pressable style={s.addBtn} onPress={() => setShowEventPicker(true)}>
            <Feather name="plus" size={12} color={C.text} />
      ...
      {hasArraySlots ? (
        ...
        >
          ...
          {arraySlotConfig!.primarySlot && (() => {
            ...
            const primaryChildren = sel.children.filter((c: any) => !c.slotName || c.slotName === slot.prop);
          ...
          {arraySlotConfig!.namedArraySlots.map(slot => {
            const slotChildren = sel.children.filter((c: any) => c.slotName === slot.prop);
...
const s = StyleSheet.create({
  ...
  // Context banner
  contextBanner: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 12, paddingVertical: 7, backgroundColor: C.surface2, borderBottomWidth: 1, borderBottomColor: C.surface2 },
  contextBannerText: { color: C.textSub, fontSize: 10, flex: 1 },
  // Section
  ...
  addBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 9, borderRadius: 7, borderWidth: 1, borderStyle: 'dashed' as any, borderColor: C.border, backgroundColor: C.surface2, marginBottom: 8 },
  addBtnText: { color: C.text, fontSize: 11, fontWeight: '600' },
  // Bindings
  contextNote: { flexDirection: 'row', alignItems: 'flex-start', gap: 6, backgroundColor: C.surface2, borderRadius: 6, padding: 8, borderWidth: 1, borderColor: C.surface2, marginBottom: 4 },
  contextNoteText: { color: C.textSub, fontSize: 9, flex: 1, lineHeight: 13 },
  ...
  slotDivider: { height: 1, backgroundColor: C.bg, marginVertical: 6 },
});
