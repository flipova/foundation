...
function fileIcon(path: string): { icon: React.ComponentProps<typeof Feather>['name']; color: string } {
  ...
  if (path.endsWith('.json')) return { icon: 'file-text', color: C.primary };
  if (path.endsWith('.md')) return { icon: 'book-open', color: C.muted };
  if (path.endsWith('.yml') || path.endsWith('.yaml')) return { icon: 'settings', color: C.primary };
  if (path.endsWith('.env')) return { icon: 'lock', color: C.error };
          ...
          <View key={node.path}>
            ...
            >
              ...
              <Feather
                ...
                size={11}
                color={node.isDir ? C.primary : color}
                style={{ marginRight: 5 }}
              />
  ...
  const fetchFiles = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const firstTsx = (data.files || []).find((f: GeneratedFile) => f.path.endsWith('.tsx'));
            ...
            <>
              ...
              {editing && (
                <View style={s.editBanner}>
                  <Feather name="edit-2" size={10} color={C.primary} />
...
const s = StyleSheet.create({
  ...
  editBannerText: { color: C.primary, fontSize: 10, flex: 1 },
});
