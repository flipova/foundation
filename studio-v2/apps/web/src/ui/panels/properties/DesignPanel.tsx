    ...
    <View style={s.alignWrap}>
      ...
      <View style={s.alignGrid}>
          ...
          <View key={row} style={s.alignGridRow}>
            {[0, 1, 2].map(col => {
              ...
              return (
                <Pressable
                  key={col}
                  style={[s.alignCell, isActive && { backgroundColor: C.primaryGlow, borderColor: C.primary }]}
                  onPress={() => handleCell(col, row)}
                >
                  <View style={[s.alignDot, isActive && { backgroundColor: C.primary }]} />
                </Pressable>
              );
            })}
          </View>
        ...
        <View style={s.alignDirOverlay} pointerEvents="none">
          <Feather name={isRow ? 'arrow-right' : 'arrow-down'} size={22} color={C.primaryGlow} />
        </View>
      </View>
      ...
      <View style={s.alignExtraRow}>
        {JUSTIFY_EXTRA.map(o => {
          ...
          return (
            <Pressable key={o.v}
              style={[s.alignExtraBtn, on && { backgroundColor: C.primaryGlow, borderColor: C.primary }]}
              onPress={() => on ? onJustify('flex-start') : onJustify(o.v)}>
              <Feather name={o.icon} size={11} color={on ? C.primary : C.muted} />
              <Text style={[s.alignExtraTxt, on && { color: C.primary }]}>{o.label}</Text>
            </Pressable>
          );
        })}
        {/* Baseline align */}
  ...
  return (
    <View style={s.dirGrid}>
      {opts.map(o => {
        const on = value === o.v;
        return (
          <Pressable key={o.v} style={[s.dirBtn, on && { backgroundColor: C.primary, borderColor: C.primary }]} onPress={() => onChange(o.v)}>
  ...
  return (
    <View style={s.dimWidget}>
      ...
      <View style={s.dimRow}>
        ...
        <Pressable style={[s.dimLock, locked && { backgroundColor: C.primaryGlow, borderColor: C.primary }]} onPress={() => setLocked(l => !l)}>
          <Feather name={locked ? 'lock' : 'unlock'} size={10} color={locked ? C.primary : C.muted} />
        </Pressable>
        <View style={s.dimField}>
...
const OpacityBar: React.FC<{ value: any; onChange: (v: any) => void; color?: string }> = ({ value, onChange, color = C.primary }) => {
  ...
  return (
    <View style={s.radiusWidget}>
      ...
      <View style={s.radiusPreview}>
        ...
        {/* Corner tap targets */}
        {corners.map(c => (
          <Pressable key={c.k} style={[s.cornerBtn, c.pos, corner === c.k && { backgroundColor: C.primaryGlow }]}
            onPress={() => setCorner(corner === c.k ? null : c.k)}>
            {c.v ? (
              <Text style={[s.cornerVal, corner === c.k && { color: C.primary }]}>{c.v}</Text>
            ) : (
              <Feather name={c.icon} size={10} color={corner === c.k ? C.primary : C.muted} />
            )}
          </Pressable>
        ))}
      </View>
      ...
      <View style={s.radiusAllRow}>
        ...
        {corner && (
          <View style={s.radiusCornerEdit}>
            <Text style={[s.radiusAllLbl, { color: C.primary }]}>{corner.replace('border','').replace('Radius','').replace(/([A-Z])/g, ' $1').trim()}</Text>
            <Num value={st[corner]} onChange={v => ch(corner, v)} style={[s.radiusAllInput, { borderColor: C.primary }]} color={C.primary} />
          </View>
        )}
      </View>
    </View>
    ...
    <View style={s.typoWidget}>
      ...
      <View style={s.typoToolbar}>
        <Pressable style={[s.toolBtn, (st.fontWeight === '700' || st.fontWeight === '800') && s.toolBtnOn]}
          ...
          <Text style={[s.toolBtnTxt, { fontWeight: '700' as any }, (st.fontWeight === '700' || st.fontWeight === '800') && { color: C.primary }]}>B</Text>
        </Pressable>
        <Pressable style={[s.toolBtn, st.fontStyle === 'italic' && s.toolBtnOn]}
          onPress={() => ch('fontStyle', st.fontStyle === 'italic' ? 'normal' : 'italic')}>
          <Text style={[s.toolBtnTxt, { fontStyle: 'italic' as any }, st.fontStyle === 'italic' && { color: C.primary }]}>I</Text>
        </Pressable>
        <Pressable style={[s.toolBtn, st.textDecorationLine === 'underline' && s.toolBtnOn]}
          ...
          <Text style={[s.toolBtnTxt, { textDecorationLine: 'underline' as any }, st.textDecorationLine === 'underline' && { color: C.primary }]}>U</Text>
        </Pressable>
        <View style={s.toolSep} />
        {(['left','center','right','justify'] as const).map(a => (
          <Pressable key={a} style={[s.toolBtn, (st.textAlign || 'left') === a && s.toolBtnOn]} onPress={() => ch('textAlign', a)}>
            <Feather name={`align-${a}` as any} size={11} color={(st.textAlign || 'left') === a ? C.primary : C.muted} />
          </Pressable>
        ))}
        <View style={s.toolSep} />
        {/* Transform */}
        {(['none','uppercase','lowercase'] as const).map(t => {
          ...
          return (
            <Pressable key={t} style={[s.toolBtn, (st.textTransform || 'none') === t && s.toolBtnOn]} onPress={() => ch('textTransform', t)}>
              <Text style={[s.toolBtnTxt, (st.textTransform || 'none') === t && { color: C.primary }]}>{lbl}</Text>
            </Pressable>
          );
        })}
      </View>
      ...
      <View style={s.weightSlider}>
        ...
        <View style={s.weightTrack}>
          {weights.map((w, i) => {
            ...
            return (
              <Pressable key={w} style={[s.weightStop, on && { backgroundColor: C.primary }]} onPress={() => ch('fontWeight', w)}>
...
const DesignPanel: React.FC = () => {
  ...
  return (
    <ScrollView style={s.root} contentContainerStyle={s.rootContent} showsVerticalScrollIndicator={false}>
      ...
      {/* ── LAYOUT ─────────────────────────────────────────────────────── */}
      <Section title="Layout" icon="columns" color={C.primary} open={ss.current.Layout} onToggle={() => tog('Layout')}
        ...
        <View style={s.layoutWidget}>
          ...
          <View style={s.layoutLeft}>
            ...
            <Pressable style={[s.wrapToggle, (st.flexWrap === 'wrap') && { borderColor: C.primary, backgroundColor: C.primaryGlow }]}
              onPress={() => ch('flexWrap', st.flexWrap === 'wrap' ? 'nowrap' : 'wrap')}>
              <Feather name="corner-down-right" size={11} color={st.flexWrap === 'wrap' ? C.primary : C.muted} />
              <Text style={[s.wrapToggleTxt, st.flexWrap === 'wrap' && { color: C.primary }]}>
        ...
        <Pressable style={[s.visToggle, st.display === 'none' && { borderColor: C.muted, opacity: 0.5 }]}
          ...
          <Feather name={st.display === 'none' ? 'eye-off' : 'eye'} size={11} color={st.display === 'none' ? C.muted : C.primary} />
      ...
      {/* ── DIMENSIONS ──────────────────────────────────────────────────── */}
      <Section title="Dimensions" icon="maximize" color={C.primary} open={ss.current.Dimensions} onToggle={() => tog('Dimensions')}>
        ...
        <View style={s.overflowTiles}>
          {(['visible','hidden','scroll'] as const).map(v => {
            ...
            return (
              <Pressable key={v} style={[s.overflowTile, on && { borderColor: C.primary, backgroundColor: C.primaryGlow }]}
                onPress={() => ch('overflow', v)}>
                <Feather name={v === 'visible' ? 'eye' : v === 'hidden' ? 'eye-off' : 'list'} size={16}
                  color={on ? C.primary : C.muted} />
                <Text style={[s.overflowTileTxt, on && { color: C.primary }]}>{v}</Text>
              </Pressable>
            );
          })}
        </View>
      ...
      {/* ── SPACING ─────────────────────────────────────────────────────── */}
      <Section title="Spacing" icon="move" color={C.primary} open={ss.current.Spacing} onToggle={() => tog('Spacing')}>
        <SpacingWidget st={st} ch={ch} />
      </Section>

      {/* ── ITEM ───────────────────────────────────────────────────────── */}
      <Section title="Item" icon="maximize-2" color={C.primary} open={ss.current.Item} onToggle={() => tog('Item')}>
        ...
        <View style={s.alignSelfRow}>
          {(['auto','flex-start','center','flex-end','stretch'] as const).map(v => {
            ...
            return (
              <Pressable key={v} style={[s.alignSelfBtn, on && { backgroundColor: C.primaryGlow, borderColor: C.primary }]}
                onPress={() => ch('alignSelf', v)}>
                <Feather name={icon[v] || 'circle'} size={13} color={on ? C.primary : C.muted} />
                <Text style={[s.alignSelfSub, on && { color: C.primary }]}>{lbl}</Text>
              </Pressable>
            );
          })}
        </View>
        ...
        {[['Grow', 'flexGrow', C.primary], ['Shrink', 'flexShrink', C.primary]].map(([lbl, k, col]) => {
      ...
      {/* ── POSITION ────────────────────────────────────────────────────── */}
      <Section title="Position" icon="crosshair" color={C.primary} open={ss.current.Position} onToggle={() => tog('Position')}>
        <View style={s.posTiles}>
          {(['relative','absolute'] as const).map(v => {
            const on = (st.position || 'relative') === v;
            return (
              <Pressable key={v} style={[s.posTile, on && { borderColor: C.primary, backgroundColor: C.primaryGlow }]}
                onPress={() => ch('position', v)}>
                <Feather name={v === 'relative' ? 'align-center' : 'move'} size={16} color={on ? C.primary : C.muted} />
                <Text style={[s.posTileTxt, on && { color: C.primary }]}>{v}</Text>
              </Pressable>
            );
          })}
        </View>
      ...
      {/* ── APPEARANCE ──────────────────────────────────────────────────── */}
      <Section title="Appearance" icon="droplet" color={C.primary} open={ss.current.Appearance} onToggle={() => tog('Appearance')}>
        {/* Big color swatch */}
        ...
        <OpacityBar value={st.opacity} onChange={v => ch('opacity', v)} color={st.backgroundColor as string || C.primary} />
        <View style={s.gapFields}>
      ...
      {/* ── BORDER ──────────────────────────────────────────────────────── */}
      <Section title="Border" icon="square" color={C.primary} open={ss.current.Border} onToggle={() => tog('Border')}>
        {/* Width + Color inline */}
        ...
        <View style={s.borderStyleRow}>
          {([
            ...
            return (
              <Pressable key={v} style={[s.borderStyleBtn, on && { borderColor: C.primary, backgroundColor: C.primaryGlow }]}
                onPress={() => ch('borderStyle', v)}>
                <Feather name={icon} size={14} color={on ? C.primary : C.muted} />
                <Text style={[s.borderStyleTxt, on && { color: C.primary }]}>{lbl}</Text>
              </Pressable>
            );
          })}
        </View>
      ...
      {/* ── TYPOGRAPHY ──────────────────────────────────────────────────── */}
      <Section title="Typography" icon="type" color={C.primary} open={ss.current.Typography} onToggle={() => tog('Typography')}>
        <TypoWidget st={st} ch={ch} />
      </Section>

      {/* ── EFFECTS ─────────────────────────────────────────────────────── */}
      <Section title="Effects" icon="zap" color={C.primary} open={ss.current.Effects} onToggle={() => tog('Effects')}>
        <View style={s.borderTopRow}>
          <View style={{ flex: 1 }}>
      ...
      {/* ── TRANSFORM ───────────────────────────────────────────────────── */}
      <Section title="Transform" icon="refresh-cw" color={C.primary} open={ss.current.Transform} onToggle={() => tog('Transform')}>
        {/* Rotate dial */}
        ...
        <View style={s.rotateRow}>
          {[-90, -45, -15, 0, 15, 45, 90].map(deg => {
            ...
            const on = cur === deg;
            return (
              <Pressable key={deg} style={[s.rotateTick, on && { backgroundColor: C.primary, borderColor: C.primary }]}
          ...
          {[
            ...
            <View key={ri} style={s.originRow}>
              {row.map(v => {
                ...
                return (
                  <Pressable key={v} style={[s.originDot, on && { backgroundColor: C.primary }]}
      ...
      {/* ── INTERACTION ─────────────────────────────────────────────────── */}
      <Section title="Interaction" icon="mouse-pointer" color={C.primary} open={ss.current.Interaction} onToggle={() => tog('Interaction')}>
        {/* Pointer events */}
        ...
        <View style={s.gapFields}>
          {(['auto','none','box-none','box-only'] as const).map(v => {
            ...
            return (
              <Pressable key={v} style={[s.gapField, on && { borderColor: C.primary, backgroundColor: C.primaryGlow }]}
                onPress={() => ch('pointerEvents', v)}>
                <Feather name={v === 'none' ? 'slash' : v === 'auto' ? 'mouse-pointer' : v === 'box-none' ? 'square' : 'box'} size={12} color={on ? C.primary : C.muted} />
                <Text style={[s.gapLbl, on && { color: C.primary }]}>{v}</Text>
              </Pressable>
            );
          })}
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.gapFields}>
          {(['auto','default','pointer','grab','grabbing','not-allowed','crosshair','text','move','zoom-in','zoom-out'] as const).map(v => {
            ...
            return (
              <Pressable key={v} style={[s.cursorChip, on && { borderColor: C.primary, backgroundColor: C.primaryGlow }]}
                onPress={() => ch('cursor', v)}>
                <Text style={[s.cursorChipTxt, on && { color: C.primary }]}>{v}</Text>
              </Pressable>
            );
          })}
        </ScrollView>
...
const s = StyleSheet.create({
  ...
  emptyTxt: { color: C.muted, fontSize: 11, textAlign: 'center', fontFamily: (font?.family || 'Lexend') },

  // Section
  section: { borderBottomWidth: 1, borderBottomColor: C.border },
  sHead: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: C.bg },
  sIcon: { width: 18, height: 18, borderRadius: 3, alignItems: 'center', justifyContent: 'center' },
  sTitle: { flex: 1, color: C.text, fontSize: 10, fontWeight: '500', fontFamily: (font?.family || 'Lexend'), letterSpacing: 0.2 },
  sBadge: { fontSize: 8, fontWeight: '500', fontFamily: (font?.family || 'Lexend'), opacity: 0.6 },
  sBody: { paddingHorizontal: 10, paddingTop: 8, paddingBottom: 14, gap: 8, backgroundColor: C.surface },

  microLabel: { color: C.muted, fontSize: 7, fontWeight: '600', fontFamily: (font?.family || 'Lexend'), letterSpacing: 1, textTransform: 'uppercase' as any, marginBottom: 4 },
  ...
  visToggleTxt: { color: C.primary, fontSize: 10, fontWeight: '500' },
  ...
  dimBox: { borderWidth: 1.5, borderColor: C.primary, borderRadius: 4, backgroundColor: C.primaryGlow, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 4, paddingHorizontal: 10, paddingVertical: 6 },
  dimBoxTxt: { color: C.primary, fontSize: 10, fontWeight: '600' },
  ...
  radiusBox: { width: 60, height: 50, borderWidth: 2, borderColor: C.primary, backgroundColor: C.primaryGlow },
  ...
  toolBtnOn: { backgroundColor: C.primaryGlow, borderColor: C.primary },
