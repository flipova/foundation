...
export const SnackPanel: React.FC = () => {
  ...
  // Auto-push to Snack when project changes (debounced 1000ms to allow CodePanel to generate)
  useEffect(() => {
    if (!_snack || !project) return;
    ...
    const timer = setTimeout(async () => {
      try {
        const res = await fetch(`${API}/snack`, { 
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ project, localFoundation })
        });
        if (!res.ok) return;
        ...
        console.warn('[SnackPanel] Auto-push failed:', e);
      }
    }, 1000);

    return () => clearTimeout(timer);
  }, [project, localFoundation]);
  ...
  const openInSnack = useCallback(async () => {
    ...
    try {
      // 1. Get files from API using the latest project JSON and localFoundation toggle
      const res = await fetch(`${API}/snack`, { 
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ project, localFoundation })
