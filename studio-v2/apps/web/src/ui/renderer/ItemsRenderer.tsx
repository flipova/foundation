...
import { resolveForPreview } from '@flipova/studio-core';
import { resolveNodeProps } from './useNodeResolution';
import { useStudio } from '../useStudio';
import {   } from '@flipova/studio-core';
    ...
    if (source.startsWith('$state.')) {
      ...
      if (query) {
        ...
        if (base) {
          ...
          fetch(url, { method: query.method || 'GET', headers })
            .then(r => r.json())
            .then(d => {
...
export const ItemsRenderer: React.FC<ItemsRendererProps> = ({
  ...
  const children = (node.children || []).filter((c: any) => !c.slotName || !secondarySlotNames.has(c.slotName));
