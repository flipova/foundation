...
function executeAction(
  ...
  switch (action.type as string) {
    ...
    case 'callApi': {
      ...
      if (svcConfig.headers) Object.assign(fetchOpts.headers as Record<string, string>, svcConfig.headers);
      if (svcConfig.apiKey) {
        if (svc?.type === 'supabase') {
          (fetchOpts.headers as Record<string, string>)['apikey'] = svcConfig.apiKey;
          (fetchOpts.headers as Record<string, string>)['Authorization'] = `Bearer ${svcConfig.apiKey}`;
        } else {
          (fetchOpts.headers as Record<string, string>)['Authorization'] = `Bearer ${svcConfig.apiKey}`;
        }
      }
      if (query.headers) Object.assign(fetchOpts.headers as Record<string, string>, query.headers);
...
export const PageQueryProvider: React.FC<{ node: TreeNode; depth?: number }> = ({ node, depth = 0 }) => {
  ...
  React.useEffect(() => {
    ...
    queries.forEach(async (query: any) => {
      ...
      if (query.headers) Object.assign(headers, query.headers);
      
      try {
        let url = base.replace(/\/$/, '') + query.path;
        if (query.params) url += (url.includes('?') ? '&' : '?') + Object.entries(query.params).map(([a, b]) => `${a}=${b}`).join('&');
        const res = await fetch(url, { method: query.method || 'GET', headers });
        if (res.ok) {
...
const NodeRenderer: React.FC<{
  ...
  if (!Component) {
    ...
    for (const secondarySlot of slotCfg.secondaryArraySlots ?? []) {
      const slotChildren = children.filter((c: any) => c.slotName === secondarySlot.prop);
      if (slotChildren.length > 0) {
        secondarySlotProps[secondarySlot.prop] = slotChildren.map((c: any) => (
    ...
    for (const slot of slotCfg.slots) {
      ...
      const matched = children.filter((c: any) => c.slotName === slot.prop);
      ...
      if (matched.length > 0) {
        if (isArraySlot) {
          ...
          if (slotBinding && slotBinding.mode !== 'static' && slotBinding.source) {
            ...
            slotProps[slot.prop] = matched.map((c: any) => (
              <NodeRenderer key={c.id} node={c} depth={depth + 1} itemContext={itemContext} queryContext={mergedQueryContext} globalContext={globalContext} themeColors={themeColors} nodePropsContext={nodePropsContext} onQueryContextUpdate={onQueryContextUpdate} />
            ));
          }
        } else {
        ...
        matched.forEach((c: any) => usedIds.add(c.id));
      } else {
        slotProps[slot.prop] = isArraySlot ? [] : <InsertZone parentId={node.id} index={children.length} slotName={slot.prop} label={slot.label} />;
      }
    }

    const unmatched = children.filter((c: any) => !usedIds.has(c.id) && (!c.slotName || c.slotName === 'children'));
    const jsxChildren = childrenSlotExists ? (
      ...
      <>{unmatched.map((c: any) => <NodeRenderer key={c.id} node={c} depth={depth + 1} itemContext={itemContext} queryContext={mergedQueryContext} globalContext={globalContext} themeColors={themeColors} nodePropsContext={nodePropsContext} onQueryContextUpdate={onQueryContextUpdate} />)}</>
    ) : null;
...
const s = StyleSheet.create({
  ...
  resizeDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#3b82f6', borderWidth: 2, borderColor: '#fff', boxShadow: '0px 0px 2px rgba(0,0,0,0.3)' as any, elevation: 4 },
  resizeDotCorner: { width: 12, height: 12, borderRadius: 3, backgroundColor: '#3b82f6', borderWidth: 2, borderColor: '#fff', boxShadow: '0px 0px 2px rgba(0,0,0,0.3)' as any, elevation: 4 },
  resizeDotActive: { backgroundColor: '#2563eb', transform: [{ scale: 1.2 }] },
  ...
  moveDot: { width: 16, height: 16, borderRadius: 8, backgroundColor: '#f59e0b', alignItems: 'center', justifyContent: 'center', boxShadow: '0px 0px 2px rgba(0,0,0,0.3)' as any, elevation: 4 },
