...
import { getAllTokens, getThemeColors } from '@flipova/studio-core';
...
const TAB_META: Record<LinkerTab, { label: string; color: string; icon: React.ComponentProps<typeof Feather>['name']; hint: string }> = {
  ITEM:   { label: 'Item',   color: colors.primary,  icon: 'repeat',       hint: 'Fields from the current list item — connect directly to item data' },
  STATE:  { label: 'State',  color: colors.text,     icon: 'box',          hint: 'Page variables — store data, API responses, user input' },
  GLOBAL: { label: 'Global', color: colors.textSub,  icon: 'globe',        hint: 'Global variables — shared across all screens' },
  NODES:  { label: 'Nodes',  color: colors.textSub,  icon: 'link',         hint: 'Reference another element\'s value reactively ($node.id.prop)' },
  THEME:  { label: 'Theme',  color: colors.primary,  icon: 'droplet',      hint: 'Theme colors — adapt to light/dark mode automatically' },
  TOK:    { label: 'Tokens', color: colors.primary,  icon: 'grid',         hint: 'Design tokens — spacing, radii, shadows, typography' },
  NAV:    { label: 'Nav',    color: colors.textSub,  icon: 'navigation',   hint: 'Navigation — current route, params, go back' },
  CFG:    { label: 'Config', color: colors.muted,    icon: 'settings',     hint: 'App constants and environment variables' },
  DEV:    { label: 'Device', color: colors.text,     icon: 'smartphone',   hint: 'Device info — screen size, platform, current date' },
};
...
const LinkerModal: React.FC<{
  ...
  const options = useMemo(() => {
    ...
    // THEME
    if (tab === 'THEME') {
      const tc = getThemeColors(project?.theme || 'light');
...
const s = StyleSheet.create({
  ...
  segRow: { gap: 4 },
  seg: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 4, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border },
  segOn: { backgroundColor: C.primary, borderColor: C.primary },
  segText: { color: C.muted, fontSize: 10, fontWeight: '500' },
  segTextOn: { color: '#fff' },
  ...
  colorInput: { flex: 1, height: 28, backgroundColor: C.surface2, borderRadius: 6, borderWidth: 1, borderColor: C.border, color: C.text, fontSize: 11, paddingHorizontal: 8 },
  numRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  numBtn: { width: 26, height: 26, borderRadius: 4, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, alignItems: 'center', justifyContent: 'center' },
  numInput: { width: 50, height: 26, backgroundColor: C.surface2, borderRadius: 4, borderWidth: 1, borderColor: C.border, color: C.text, fontSize: 11, textAlign: 'center' },
  textInput: { height: 28, backgroundColor: C.surface2, borderRadius: 6, borderWidth: 1, borderColor: C.border, color: C.text, fontSize: 11, paddingHorizontal: 10 },
  linkBtn: { width: 26, height: 26, borderRadius: 4, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, alignItems: 'center', justifyContent: 'center' },
  linkBtnActive: { backgroundColor: C.primaryGlow, borderColor: C.cyan },
  linkedBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: C.primaryGlow, borderRadius: 4, paddingHorizontal: 8, paddingVertical: 4, borderWidth: 1, borderColor: C.border },
  linkedBadgeItem: { backgroundColor: C.primaryGlow, borderColor: C.border },
  ...
  linkerPropType: { color: C.muted, fontSize: 10, backgroundColor: C.surface2, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  linkerTabs: { gap: 4, paddingHorizontal: 10, paddingVertical: 8 },
  linkerTab: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 4, backgroundColor: C.surface2 },
  linkerTabText: { color: C.muted, fontSize: 10, fontWeight: '600' },
  linkerTabTextOn: { color: '#fff' },
  linkerSearch: { height: 34, backgroundColor: C.surface2, borderBottomWidth: 1, borderBottomColor: C.border, color: C.text, fontSize: 12, paddingHorizontal: 12 },
  ...
  // Tab hint
  tabHint: { paddingHorizontal: 12, paddingVertical: 4, backgroundColor: C.surface2, borderBottomWidth: 1, borderBottomColor: C.border },
  tabHintText: { color: C.muted, fontSize: 9, fontStyle: 'italic' },
  // Indented items (drill-down fields)
  linkerItemIndented: { paddingLeft: 24 },
  // Response badge
  responseBadge: { backgroundColor: C.primaryGlow, borderRadius: 3, paddingHorizontal: 5, paddingVertical: 1 },
  responseBadgeText: { color: '#f97316', fontSize: 8, fontWeight: '600' },
  // Query alias badge
  queryAliasBadge: { backgroundColor: C.primaryGlow, borderRadius: 3, paddingHorizontal: 5, paddingVertical: 1 },
  queryAliasBadgeText: { color: '#06b6d4', fontSize: 8, fontWeight: '600' },
  // Node prop badge
  nodePropBadge: { backgroundColor: C.primaryGlow, borderRadius: 3, paddingHorizontal: 5, paddingVertical: 1 },
  nodePropBadgeText: { color: '#f97316', fontSize: 8, fontWeight: '600' },
  // Node group header
  linkerGroupHeader: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingHorizontal: 12, paddingVertical: 5, backgroundColor: C.surface2, borderBottomWidth: 1, borderBottomColor: C.border },
