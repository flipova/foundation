...
import { FOUNDATION_TOKENS as TOKENS, AVAILABLE_THEMES, getThemeColors  } from '@flipova/studio-core';
            ...
            <View style={s.colorGrid}>
              {Object.entries(themeColors).map(([role, color]) => (
                <View key={role} style={s.colorItem}>
                  ...
                  <TextInput style={s.colorInput} value={overrides[role] || color} onChangeText={(v: any) => setOverride(role, v)} placeholderTextColor={C.muted} />
                </View>
              ))}
            </View>
          </>
...
const s = StyleSheet.create({
  ...
  sidebarText: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend') },
  sidebarTextOn: { color: C.text, fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  content: { flex: 1 },
  title: { color: C.text, fontSize: (font?.size?.lg || 14), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend'), marginBottom: 12 },
  hint: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend'), marginBottom: 12 },
  themeBadge: { backgroundColor: C.primary, paddingHorizontal: 10, paddingVertical: 3, borderRadius: (radius?.lg || 6) },
  themeBadgeText: { color: '#fff', fontSize: (font?.size?.sm || 11), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  // Theme grid
  themeGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  themeCard: { width: 120, backgroundColor: C.surface2, borderRadius: 10, borderWidth: 1, borderColor: C.border, padding: 10, alignItems: 'center', gap: 6 },
  ...
  themeName: { color: C.muted, fontSize: (font?.size?.sm || 11), fontWeight: (font?.weight?.semi || '500'), fontFamily: (font?.family || 'Lexend'), textTransform: 'capitalize' as any },
  themeNameOn: { color: C.primary, fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  // Color grid
  colorGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  colorItem: { width: 95, alignItems: 'center', gap: 3 },
  colorSwatch: { width: 40, height: 40, borderRadius: (radius?.xl || 8), borderWidth: 1, borderColor: C.border },
  colorLabel: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend'), textAlign: 'center' },
  colorInput: { width: 85, height: 22, backgroundColor: C.surface2, borderRadius: (radius?.md || 4), borderWidth: 1, borderColor: C.border, color: C.text, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend'), textAlign: 'center', paddingHorizontal: 4 },
  // Token grid
  tokenGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  tokenItem: { width: 110, backgroundColor: C.surface2, borderRadius: (radius?.lg || 6), borderWidth: 1, borderColor: C.border, padding: 8, gap: 2 },
  tokenKey: { color: C.text, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  tokenVal: { color: C.primary, fontSize: (font?.size?.md || 12), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  tokenPath: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend'), opacity: 0.7 },
  miniSwatch: { width: 16, height: 16, borderRadius: (radius?.md || 4), borderWidth: 1, borderColor: C.border },
});
