...
`;
  ...
  return (
    <ModalShell title="Custom Function Builder" icon="code" iconColor={colors.primary} onClose={onClose} width="88%" height="88%">
      {/* Step indicator */}
      <View style={m.steps}>
            ...
            {params.map((p, i) => (
              <View key={i} style={m.paramRow}>
                ...
                <SmartInput
                  ...
                  onChange={(v: any) => setParams(prev => prev.map((pp, j) => j === i ? { ...pp, stateBinding: v } : pp))}
                  propType="string"
                  isExpression
...
const m = StyleSheet.create({
  ...
  stepActive: {},
  stepDot: { width: 20, height: 20, borderRadius: 10, backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, alignItems: 'center', justifyContent: 'center' },
  stepDotActive: { backgroundColor: C.primary, borderColor: C.primary },
  stepNum: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  stepLabel: { color: C.muted, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.semi || '500'), fontFamily: (font?.family || 'Lexend') },
  // Template grid
  templateGrid: { padding: 20, gap: 12 },
  stepTitle: { color: C.text, fontSize: (font?.size?.lg || 14), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  stepDesc: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend'), lineHeight: 15 },
  grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  templateCard: { width: '47%', backgroundColor: C.surface2, borderRadius: 10, borderWidth: 1, borderColor: C.border, padding: 14, gap: 6 },
  templateIcon: { width: 40, height: 40, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  templateLabel: { color: C.text, fontSize: (font?.size?.md || 12), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  templateDesc: { color: C.muted, fontSize: (font?.size?.xs || 10), fontFamily: (font?.family || 'Lexend'), lineHeight: 14 },
  templateArrow: { alignSelf: 'flex-end' },
  // Configure
  templateBadge: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: C.surface2, borderRadius: (radius?.xl || 8), padding: 10, borderWidth: 1, borderColor: C.border },
  templateBadgeText: { fontSize: (font?.size?.md || 12), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend'), flex: 1 },
  changeTplBtn: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: (radius?.md || 4), backgroundColor: C.bg, borderWidth: 1, borderColor: C.border },
  changeTplText: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend') },
  fieldLabel: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend'), marginBottom: 6 },
  sectionTitle: { color: C.text, fontSize: (font?.size?.md || 12), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend'), marginBottom: 4 },
  sectionDesc: { color: C.muted, fontSize: (font?.size?.xs || 10), fontFamily: (font?.family || 'Lexend'), lineHeight: 14, marginBottom: 8 },
  paramRow: { backgroundColor: C.surface2, borderRadius: (radius?.xl || 8), borderWidth: 1, borderColor: C.border, padding: 10, marginBottom: 8, gap: 6 },
  paramHeader: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  paramName: { color: C.text, fontSize: (font?.size?.sm || 11), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend'), flex: 1 },
  paramType: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend'), backgroundColor: C.bg, paddingHorizontal: 5, paddingVertical: 2, borderRadius: (radius?.md || 4) },
  addParamBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 6 },
  addParamText: { color: C.primary, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.semi || '500'), fontFamily: (font?.family || 'Lexend') },
  ...
  svcBtnText: { color: C.muted, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.semi || '500'), fontFamily: (font?.family || 'Lexend') },
  codeEditor: { backgroundColor: C.bg, borderRadius: (radius?.xl || 8), borderWidth: 1, borderColor: C.border, color: C.text, fontSize: (font?.size?.sm || 11), fontFamily: 'monospace' as any, padding: 12, minHeight: 160, textAlignVertical: 'top' },
  nextBtn: { backgroundColor: C.primary, borderRadius: (radius?.xl || 8), paddingVertical: 12, alignItems: 'center' },
  nextBtnText: { color: '#fff', fontSize: (font?.size?.base || 13), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  // Preview
  ...
  previewTitle: { color: C.text, fontSize: (font?.size?.md || 12), fontWeight: (font?.weight?.bold || '600'), fontFamily: 'monospace' as any },
  previewDesc: { color: C.muted, fontSize: (font?.size?.xs || 10), fontFamily: (font?.family || 'Lexend'), marginTop: 2 },
  copyBtn: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: C.primary, paddingHorizontal: 12, paddingVertical: 6, borderRadius: (radius?.lg || 6) },
  copyBtnDone: { backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border },
  copyBtnText: { color: '#fff', fontSize: (font?.size?.sm || 11), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  codeScroll: { flex: 1 },
  codeText: { color: C.text, fontSize: (font?.size?.sm || 11), fontFamily: 'monospace' as any, lineHeight: 18 },
  usageBox: { backgroundColor: C.surface2, borderTopWidth: 1, borderTopColor: C.border, padding: 14, gap: 6 },
  usageTitle: { color: C.primary, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  usageCode: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: 'monospace' as any, lineHeight: 15 },
});
