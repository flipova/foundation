...
import { DataQuery } from '@flipova/studio-core';
...
const METHOD_COLORS: Record<string, string> = {
  GET: colors.text, POST: colors.primary, PUT: colors.textSub, PATCH: colors.textSub, DELETE: colors.error,
};
...
const DataNode: React.FC<{ k: string; v: any; depth: number; path: string; onCopy: (p: string) => void }> = ({ k, v, depth, path, onCopy }) => {
  ...
  const tc = isArr ? C.warning : isObj ? C.info : typeof v === 'boolean' ? C.primary : typeof v === 'number' ? C.success : C.text;
  return (
    <View>
...
const QueryPreview: React.FC<{ query: DataQuery; baseUrl: string; activeSvc?: any; onCopied: (e: string) => void }> = ({ query, baseUrl, activeSvc, onCopied }) => {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const svcConfig = activeSvc?.config || {};
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (svcConfig.headers) Object.assign(headers, svcConfig.headers);
      if (svcConfig.apiKey) {
        if (activeSvc?.type === 'supabase') {
          headers['apikey'] = svcConfig.apiKey;
          headers['Authorization'] = `Bearer ${svcConfig.apiKey}`;
        } else {
          headers['Authorization'] = `Bearer ${svcConfig.apiKey}`;
        }
      }
...
const DataQueryModal: React.FC<Props> = ({ onClose }) => {
  ...
  const save = useCallback(() => {
    ...
    const { normalizeQueryName } = require('@flipova/studio-engine');
  ...
  return (
    <ModalShell title="Data Queries" icon="database" onClose={onClose} width="90%" height="85%"
      headerRight={copied ? <View style={m.copiedBadge}><Feather name="link" size={9} color={C.info} /><Text style={m.copiedText} numberOfLines={1}>{copied}</Text></View> : undefined}>
      <View style={m.body}>
        {/* Sidebar */}
        ...
        <View style={m.main}>
          ...
          {tab === 'edit' ? (
            <ScrollView style={m.form} contentContainerStyle={{ padding: 16 }}>
              ...
              <View style={m.aliasBox}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                  <Feather name="box" size={12} color={C.primary} />
                  <Text style={[m.fieldLabel, { color: C.primary, marginBottom: 0 }]}>State Alias</Text>
            ...
            <QueryPreview query={activeQuery} baseUrl={baseUrl} activeSvc={activeSvc} onCopied={e => setCopied(e)} />
          ) : (
...
const m = StyleSheet.create({
  ...
  qRowOn: { backgroundColor: C.surface2 },
  dot: { width: 8, height: 8, borderRadius: (radius?.md || 4), flexShrink: 0 },
  qName: { color: C.text, fontSize: (font?.size?.sm || 11), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  qMeta: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend') },
  qAlias: { color: C.primary, fontSize: (font?.size?.xxs || 8), fontFamily: 'monospace' as any },
  emptyList: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend'), textAlign: 'center', padding: 16 },
  ...
  tabBtnOn: { backgroundColor: C.surface2, borderBottomWidth: 2, borderBottomColor: C.primary },
  tabText: { color: C.muted, fontSize: (font?.size?.sm || 11), fontWeight: (font?.weight?.semi || '500'), fontFamily: (font?.family || 'Lexend') },
  tabTextOn: { color: C.text, fontFamily: (font?.family || 'Lexend') },
  form: { flex: 1 },
  field: { marginBottom: 10 },
  fieldLabel: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend'), marginBottom: 3 },
  svcBtn: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: (radius?.lg || 6), backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border },
  svcBtnOn: { backgroundColor: C.primary, borderColor: C.primary },
  svcText: { color: C.muted, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.semi || '500'), fontFamily: (font?.family || 'Lexend') },
  svcTextOn: { color: '#fff', fontFamily: (font?.family || 'Lexend') },
  noSvc: { color: C.muted, fontSize: (font?.size?.xs || 10), fontFamily: (font?.family || 'Lexend'), fontStyle: 'italic' },
  methodBtn: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: (radius?.md || 4), backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border },
  methodText: { color: C.muted, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  aliasBox: { backgroundColor: C.surface2, borderRadius: (radius?.xl || 8), borderWidth: 1, borderColor: C.border, padding: 10, marginBottom: 12 },
  aliasExpr: { color: C.primary, fontSize: (font?.size?.xxs || 8), fontFamily: 'monospace' as any, flex: 1 },
  aliasInput: { height: 30, backgroundColor: C.bg, borderRadius: (radius?.lg || 6), borderWidth: 1, borderColor: C.border, color: C.text, fontSize: (font?.size?.md || 12), fontFamily: (font?.family || 'Lexend'), paddingHorizontal: 8 },
  aliasDesc: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend'), marginTop: 5, fontStyle: 'italic' },
  ...
  copiedBadge: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: C.surface2, borderRadius: (radius?.md || 4), paddingHorizontal: 8, paddingVertical: 3, marginHorizontal: 8 },
  copiedText: { color: C.primary, fontSize: (font?.size?.xs || 10), fontFamily: 'monospace' as any, flex: 1 },
});
const pv = StyleSheet.create({
  root: { flex: 1 },
  header: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 10, borderBottomWidth: 1, borderBottomColor: C.border },
  badge: { paddingHorizontal: 5, paddingVertical: 2, borderRadius: (radius?.sm || 3) },
  badgeText: { color: '#fff', fontSize: (font?.size?.xxs || 8), fontWeight: (font?.weight?.bold || '600'), fontFamily: (font?.family || 'Lexend') },
  url: { flex: 1, color: C.muted, fontSize: (font?.size?.xs || 10), fontFamily: 'monospace' as any },
  aliasBadge: { backgroundColor: C.surface2, borderRadius: (radius?.md || 4), paddingHorizontal: 6, paddingVertical: 2 },
  aliasText: { color: C.primary, fontSize: (font?.size?.xxs || 8), fontFamily: 'monospace' as any },
  runBtn: { width: 26, height: 26, borderRadius: (radius?.lg || 6), backgroundColor: C.primary, alignItems: 'center', justifyContent: 'center' },
  flash: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: C.surface2, padding: 8, borderBottomWidth: 1, borderBottomColor: C.border },
  flashText: { color: C.text, fontSize: (font?.size?.xs || 10), fontFamily: 'monospace' as any },
  err: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: C.surface, padding: 8 },
  errText: { color: C.error, fontSize: (font?.size?.xs || 10), fontFamily: (font?.family || 'Lexend') },
  tree: { flex: 1 },
  hint: { color: C.muted, fontSize: (font?.size?.xxs || 8), fontFamily: (font?.family || 'Lexend'), fontStyle: 'italic', padding: 8, borderBottomWidth: 1, borderBottomColor: C.border },
  empty: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  emptyText: { color: C.muted, fontSize: (font?.size?.sm || 11), fontFamily: (font?.family || 'Lexend') },
});
const t = StyleSheet.create({
  row: { flexDirection: 'row', alignItems: 'center', paddingVertical: 4, paddingRight: 8, borderBottomWidth: 1, borderBottomColor: C.border },
  key: { color: C.primary, fontSize: (font?.size?.xs || 10), fontWeight: (font?.weight?.bold || '600'), fontFamily: 'monospace' as any },
  colon: { color: C.muted, fontSize: (font?.size?.xs || 10), fontFamily: (font?.family || 'Lexend') },
  val: { flex: 1, fontSize: (font?.size?.xs || 10), fontFamily: 'monospace' as any },
  copy: { padding: 3 },
});
