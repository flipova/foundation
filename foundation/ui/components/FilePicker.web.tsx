...
export interface FilePickerProps extends Omit<Omit<React.InputHTMLAttributes<HTMLInputElement>, "size" | "type">, "color" | "size">, ExtractComponentProps<"FilePicker"> {
...
const FilePicker: React.FC<FilePickerProps> = (rawProps) => {
  ...
  const {
    label, variant, size, accept, multiple, disabled, borderRadius,
    onFilesChange, onChange, style, ...rest
  } = applyDefaults(rawProps, META, theme) as any;

  const inputRef = useRef<HTMLInputElement>(null);
  const [isDragging, setIsDragging] = useState(false);
  const [fileNames, setFileNames] = useState<string[]>([]);
  const radius = radii[borderRadius as RadiusToken] || radii.md;
      ...
      <div>
        ...
        >
          ...
          <p style={{ margin: "0 0 4px", fontSize: 14, fontWeight: 600, color: theme.foreground, fontFamily: "inherit" }}>
            {fileNames.length > 0 ? fileNames.join(", ") : label || "Upload file"}
          </p>
