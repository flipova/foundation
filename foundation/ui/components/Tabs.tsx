...
export interface TabsProps extends ExtractComponentProps<"Tabs"> {
  items: TabItem[];
  defaultValue?: string;
  value?: string;
...
const Tabs: React.FC<TabsProps> = (rawProps) => {
  ...
  const merged = applyDefaults(rawProps, META, theme) as Required<TabsProps> & typeof rawProps;
  const {
    items,
    defaultValue,
    value,
  ...
  const radius = borderRadius ? radii[borderRadius as RadiusToken] : radii.none;
