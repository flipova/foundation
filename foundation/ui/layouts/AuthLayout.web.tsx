...
export interface AuthLayoutProps extends Omit<React.HTMLAttributes<HTMLDivElement>, "color" | "size">, ExtractLayoutProps<"AuthLayout"> {
...
const AuthLayout: React.FC<AuthLayoutProps> = (rawProps) => {
  ...
  const containerStyle: CSSProperties = {
    ...
    minHeight: "100vh",
    width: "100%",
    backgroundColor: (Array.isArray(background) ? background[0] : background) ?? theme.background,
  ...
  const brandingStyle: CSSProperties = {
    flex: ratio,
    backgroundColor: (Array.isArray(brandingBackground) ? brandingBackground[0] : brandingBackground) ?? theme.card,
    display: "flex",
    alignItems: "center",
